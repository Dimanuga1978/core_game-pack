...
export const yourGame = {
  ...
  // applyAction НЕ нужно писать вручную, если реализуете
  // applyActionInPlace ниже — движок синтезирует его автоматически.
  // См. §5.2.
...
### 5.2. `applyAction` vs `applyActionInPlace` — и почему `applyAction` не нужно писать вручную
...
- **Если реализован только `applyAction(state, action, context)` без `applyActionInPlace`** — движок сам делает `structuredClone(state)` и передаёт **обычный, не-draft объект**.

`applyAction` **обязателен** как часть публичного контракта (`packages/game-pack/src/index.js`'s `requiredGame`) — но если вы реализуете `applyActionInPlace`, **не пишите `applyAction` вручную**. Используйте общий, готовый синтез:
...
const rules = {
  createInitialState(...) { /* ... */ },
  getLegalActions(...) { /* ... */ },
  applyActionInPlace(state, action, context = {}) { /* мутируете draft напрямую */ },
  getGameStatus(...) { /* ... */ },
};

export const yourGame = { ...rules, applyAction: synthesizeApplyAction(rules.applyActionInPlace) };
```

### ⚠️ Реальная, уже дважды случившаяся история этого правила

Раньше в этом движке `applyAction` нужно было писать вручную как тонкую обёртку:
```js
applyAction(state, action, context = {}) { return this.applyActionInPlace(structuredClone(state), action, context); },
```

Это была **одна и та же строка, продублированная вручную во всех пяти реальных играх этого проекта**. Канонический диспетчер (`runAction.js`) всегда вызывает `game.applyAction(state, action, context)` **тремя** аргументами — и **три из пяти игр** независимо друг от друга ошиблись именно на этом: приняли только два параметра и жёстко подставили `context: {}`, теряя реально переданный контекст (включая `rng`). Поскольку игра с реальной случайностью (`last-sector`) безусловно требует `context.rng`, прямой вызов её `applyAction()` падал с `RNG_CONTEXT_REQUIRED` для **любого** действия.

Первое исправление (вручную поправить все пять копий) устранило симптом, но не причину — идентичный код, продублированный вручную в N местах, будет дублировать и ошибки. **Правильное исправление**: убрать ручной код полностью, вынести делегацию в `@tablecore/core`'s `synthesizeApplyAction()` — единственную, переиспользуемую, протестированную реализацию. При её написании нашёлся ещё один, более тонкий баг: события (`events`), построенные внутри `applyActionInPlace`, могут содержать **живые ссылки** на под-объекты драфта (например, координаты `from`/`to`, взятые до мутации) — без `JSON.parse(JSON.stringify(events))` (той же защиты, что уже есть в реальном пути `runAction.js`) чтение таких событий после завершения `produce()` бросает `TypeError: Cannot perform 'get' on a proxy that has been revoked`.

`createGamePack()` дополнительно синтезирует `applyAction` **автоматически**, если вы предоставили `applyActionInPlace`, но не написали `applyAction` вовсе (второй уровень защиты — на случай, если вы не подключаете `synthesizeApplyAction` явно на уровне `src/game.js`, что рекомендуется, если сырой экспорт вашей игры используется где-либо напрямую, не только через пак).

Движок автоматически проверяет корректность синтеза для **всех** зарегистрированных игр (см. `packages/game-api/test/applyActionContextForwarding.test.js`) — сравнением результата `game.applyAction(...)` с эталонным `synthesizeApplyAction(game.applyActionInPlace)(...)` на одинаковых входных данных.
...
### 5.7. ⚠️ `Map`/`Set` в состоянии молча теряются при отправке клиенту, если не спроецированы в обычные структуры

**Реальная, проверенная находка.** Весь протокол движка использует **JSON** как формат передачи по проводу (WS-сообщения) — а `JSON.stringify()` не умеет сериализовать `Map`/`Set` вообще: `JSON.stringify(new Map([['x',1]]))` даёт `"{}"` — **пустой объект, без единой ошибки или предупреждения**. Внутри самого движка (`ServerHost.getSnapshot()`) `Map`/`Set` сохраняются корректно как реальные объекты в памяти — потеря происходит **только** на финальном шаге сериализации в реальное сообщение клиенту.

Подтверждено прямым тестом: гипотетическая игра с `state.tiles = new Map(...)`, не реализующая `getPlayerView()` (не обязателен, если `manifest.hiddenInformation` не `true`) — реальный снапшот, отправленный клиенту, содержит `tiles: {}` вместо настоящих данных. **Никакого исключения, никакого предупреждения** — ни от движка, ни от линтера (это невозможно проверить статически без реального вызова `createInitialState()` и рекурсивного обхода результата).

`games/last-sector` (единственная реальная игра, использующая `Map`/`Set` как основной внутренний формат для `tiles`/`units`) **уже корректно** конвертирует их в обычные массивы внутри собственного `getPlayerView()` — именно поэтому проблема никогда не проявлялась в реальном, поставляемом коде. Но если ваша игра использует `Map`/`Set` в состоянии и **не** реализует `getPlayerView()` (например, потому что `hiddenInformation: false` и вы посчитали проекцию не нужной) — данные внутри `Map`/`Set` реально дойдут до клиента как пустой объект.

**Правило**: если состояние вашей игры содержит `Map`/`Set` где-либо (даже вложенно), либо конвертируйте их в массивы/обычные объекты внутри `getPlayerView()` (даже если проекция иначе не нужна — тогда `getPlayerView` становится чистым сериализатором, не фильтром видимости), либо не используйте `Map`/`Set` в частях состояния, которые должны быть видны клиенту.
...
- [ ] Если реализуете `applyActionInPlace` — **не** пишите `applyAction` вручную; экспортируйте `{ ...rules, applyAction: synthesizeApplyAction(rules.applyActionInPlace) }` из `@tablecore/core`.
- [ ] Если состояние вашей игры содержит `Map`/`Set` где-либо (даже вложенно) — конвертированы в массивы/объекты внутри `getPlayerView()`, иначе молча теряются при отправке клиенту (см. §5.7).
